...
let activeCallPageInstance = null;

if (import.meta.hot) {
  import.meta.hot.dispose(() => {
    activeCallPageInstance = null;
  });
...
const CallPage = () => {
  ...
  const pageInstanceKey = useMemo(() => {
    return `${callId || "no-call"}:${peerId || "no-peer"}:${isInitiator ? "init" : "recv"}`;
  }, [callId, peerId, isInitiator]);
  ...
  useEffect(() => {
    ...
    if (activeCallPageInstance && activeCallPageInstance !== pageInstanceKey) {
      console.log("[CALL PAGE] Duplicate page instance blocked:", pageInstanceKey);
      setIsDuplicatePage(true);
      return;
    }

    activeCallPageInstance = pageInstanceKey;
    setIsDuplicatePage(false);

    return () => {
      if (activeCallPageInstance === pageInstanceKey) {
        activeCallPageInstance = null;
      }
    };
  }, [pageInstanceKey]);
