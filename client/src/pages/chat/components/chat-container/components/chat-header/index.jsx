...
const ChatHeader = () => {
  ...
  const availableFriends = useMemo(() => {
    return friendOptions.filter((friend) => !memberIds.has(String(friend.value)));
  }, [friendOptions, memberIds]);
  ...
  const fetchFriendOptions = useCallback(async () => {
    try {
  ...
  }, [channelDialogOpen, fetchFriendOptions, fetchChannelDetails]);
  ...
  const addMembers = async () => {
    if (!selectedChannelId || !selectedFriends.length) {
      return;
    }
    ...
    try {
      ...
      const response = await apiClient.post(
        ...
        { memberIds: selectedFriends.map((friend) => friend.value) },
        { withCredentials: true }
      );
              ...
              <div className="rounded-xl bg-muted p-4">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <Button
                    className="bg-green-700 hover:bg-green-900 transition-all duration-200"
                    onClick={addMembers}
                    disabled={isBusy || !selectedFriends.length}
                  >
